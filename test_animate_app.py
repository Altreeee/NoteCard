...
class SampleApp(tk.Tk):
    ...
    def show_welcome(self):
        ...
        for item in self.items:
            button = tk.Button(frameyouxia, image=self.photo_images[1], text=item[0], command=lambda id=i,work_name=item[0]: (self.WeakAWork(work_name),self.toggle_state(id)))
...
###########################################################################################################################
    def ctrl_bottonanimate(self,id):
    ...
    def toggle_state(self,id):
        # 切换按钮状态
        self.button = self.buttons[id]
        self.button.config(image=self.photo_images[0])  # 按下时显示第二张图片
        
    def restore_state(self,id):
        # 恢复按钮状态
        self.button = self.buttons[id]
        self.button.config(image=self.photo_images[1])  # 恢复显示第一张图片
